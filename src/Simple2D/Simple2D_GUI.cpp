...
namespace Simple2D {
	...
	namespace GUI {
		...
		pop_up_window::pop_up_window(const Simple2D::Context& ctx, float pos_x_, float pos_y_, float scaled_pos_x_, float scaled_width_, float width_, float height_, int animation_time_, const Simple2D::Colour& colour_, const Simple2D::Colour& background_colour_, const std::string& title_, const std::string& internal_name_)
			...
			internal_name(internal_name_) {
				for(int i = 0; i <= animation_time; i++) {
					float start_x       = pos_x-((pos_x-scaled_pos_x)/animation_time)*i;
					float new_width     = width+((scaled_width-width)/animation_time)*i;
					float start_y       = pos_y-((pos_y-height)/animation_time)*i;
					float new_height    = pos_y-start_y;
					int   bar_height    = ceil(new_height*0.05);
					int   window_height = ceil(new_height*0.95);
					int   text_size     = (new_height*0.05)/1.33333;

					animation_steps.push_back({start_x, start_y, new_width, new_height, bar_height, window_height, text_size});
				}		
			};
		...
		void pop_up_window::draw_scale_up(Simple2D::Text_context& text_ctx, const Simple2D::Context& ctx) {
			float start_x    = animation_steps[animation_step].pos_x;
			float start_y    = animation_steps[animation_step].pos_y;
			float new_width  = animation_steps[animation_step].width;
			float new_height = animation_steps[animation_step].height;

			ctx.draw_rect(start_x, start_y+animation_steps[animation_step].bar_height, floor(new_width), animation_steps[animation_step].window_height, background_colour, true);
			...
			ctx.draw_rect(start_x, start_y, floor(new_width), animation_steps[animation_step].bar_height, colour, true);
			text_ctx.draw_text(ctx, start_x+2, start_y+2, title, animation_steps[animation_step].text_size);
		}
		...
		void pop_up_window::draw_mask(const Simple2D::Context& ctx) {
			...
			ctx.draw_rect(start_x, start_y, floor(new_width), animation_steps[animation_step].bar_height, {255, 255, 255, 255}, true);
			ctx.draw_rect(start_x, start_y+animation_steps[animation_step].bar_height, floor(new_width), animation_steps[animation_step].window_height, {255, 255, 255, 255}, true);
		}
			...
			if(extended) {
				if(mouse_pos_x > start_x && mouse_pos_x < start_x+new_width) {
					if(mouse_pos_y > start_y && mouse_pos_y < pos_y) {
						...
						if(current_scroll < (ceil((float)(buttons.size()) / 3.0f)-3) * -(animation_steps[animation_step].window_height/3)) {
							current_scroll = (ceil((float)(buttons.size()) / 3.0f)-3) * -(animation_steps[animation_step].window_height/3);
						}
					}
				}
			}			
