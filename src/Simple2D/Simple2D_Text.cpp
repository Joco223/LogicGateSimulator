...
namespace Simple2D {
	...
	void Text_context::draw_text(const Context& ctx, int x, int y, const std::string& text, int size) {
		Simple2D::Colour white = {255, 255, 255, 255};
		return draw_text(ctx, x, y, text, size, white);
	}

	void Text_context::draw_text(const Context& ctx, int x, int y, const std::string& text, int size, Simple2D::Colour c) {
	...
	void Text_context::get_text_size(const Context& ctx, int& width_total, int& height_total, const std::string& text, int size) {
		std::vector<std::string> split_input = split(text);
